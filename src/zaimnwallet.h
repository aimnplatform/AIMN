...
#ifndef AIMN_ZAIMNWALLET_H
#define AIMN_ZAIMNWALLET_H
...
public:
    CzAIMNWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZAIMN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZAIMN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //AIMN_ZAIMNWALLET_H

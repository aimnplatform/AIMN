...
// Copyright (c) 2017-2019 The Aimn Developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::AIMN)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sAIMNPercentage, QString& szAIMNPercentage)
{
    int nPrecision = 2;
    ...
    szAIMNPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sAIMNPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount aimnAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = aimnAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // AIMN labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, aimnAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelAIMNPercent->setText(sPercentage);
    ui->labelzAIMNPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zAIMN.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in aimn.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in aimn.conf");
    }
    ...
    bool showAIMNAvailable = settingShowAllBalances || aimnAvailableBalance != nTotalBalance;
    bool showWatchOnlyAIMNAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showAIMNPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyAIMNPending = watchUnconfBalance != 0;
    bool showAIMNLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyAIMNLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showAIMNAvailable || showWatchOnlyAIMNAvailable);
    ui->labelBalanceText->setVisible(showAIMNAvailable || showWatchOnlyAIMNAvailable);
    ui->labelWatchAvailable->setVisible(showAIMNAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showAIMNPending || showWatchOnlyAIMNPending);
    ui->labelPendingText->setVisible(showAIMNPending || showWatchOnlyAIMNPending);
    ui->labelWatchPending->setVisible(showAIMNPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showAIMNLocked || showWatchOnlyAIMNLocked);
    ui->labelLockedBalanceText->setVisible(showAIMNLocked || showWatchOnlyAIMNLocked);
    ui->labelWatchLocked->setVisible(showAIMNLocked && showWatchOnly);
    ...
    bool showzAIMNAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzAIMNUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzAIMNImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzAIMNAvailable);
    ui->labelzBalanceMatureText->setVisible(showzAIMNAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzAIMNUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzAIMNUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzAIMNImmature);
    ui->labelzBalanceImmatureText->setVisible(showzAIMNImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelAIMNPercent->setVisible(showPercentages);
    ui->labelzAIMNPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("AIMN")
    updateDisplayUnit();
}

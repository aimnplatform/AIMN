...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::AIMN);
    ...
    if (!settings.contains("nAnonymizeAimnAmount"))
        settings.setValue("nAnonymizeAimnAmount", 1000);

    nAnonymizeAimnAmount = settings.value("nAnonymizeAimnAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAimnAmount"))
        SoftSetArg("-anonymizeaimnamount", settings.value("nAnonymizeAimnAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in aimn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAimnAmount:
            return QVariant(nAnonymizeAimnAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAimnAmount:
            nAnonymizeAimnAmount = value.toInt();
            settings.setValue("nAnonymizeAimnAmount", nAnonymizeAimnAmount);
            emit anonymizeAimnAmountChanged(nAnonymizeAimnAmount);
            break;
        case CoinControlFeatures:

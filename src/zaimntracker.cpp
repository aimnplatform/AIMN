...
CzAIMNTracker::CzAIMNTracker(std::string strWalletFile)
{
...
CzAIMNTracker::~CzAIMNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAIMNTracker::Init()
{
...
bool CzAIMNTracker::Archive(CMintMeta& meta)
{
...
bool CzAIMNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAIMNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAIMNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAIMNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzAIMNTracker::GetSerialHashes()
{
...
CAmount CzAIMNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAIMNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAIMNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAIMNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAIMNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAIMNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAIMNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAIMNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAIMNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAIMNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAIMNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAIMNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAIMNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAIMNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAIMNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAIMNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAIMNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzaimn from DB\n", __func__, listDeterministicDB.size());
    }

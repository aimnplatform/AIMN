...
CzAIMNWallet::CzAIMNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zaimn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAIMNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAIMNSeed_deprecated()) {
                    LogPrintf("%s: Updated zAIMN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zaimn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zaimn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAIMNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAIMNCount(nCountLastUsed);
    else if (!walletdb.ReadZAIMNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAIMNWallet::Lock()
{
    seedMaster = 0;
}

void CzAIMNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAIMNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAIMN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAIMNWallet::LoadMintPoolFromDB()
{
...
void CzAIMNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAIMNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAIMNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zaimnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAIMNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAIMN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zaimnTracker which also adds to database
    pwalletMain->zaimnTracker->Add(dMint, true);
...
void CzAIMNWallet::SeedToZAIMN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAIMNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAIMNWallet::GenerateDeterministicZAIMN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAIMNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAIMNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

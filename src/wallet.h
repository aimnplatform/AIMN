...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 AIMN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAIMN send
enum ZerocoinSpendStatus {
    ZAIMN_SPEND_OKAY = 0,                            // No error
    ZAIMN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAIMN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAIMN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAIMN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAIMN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAIMN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAIMN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAIMN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAIMN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAIMN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAIMN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAIMN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAIMN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAIMN_SPENT_USED_ZAIMN = 14,                      // Coin has already been spend
    ZAIMN_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZAIMN_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZAIMNOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzaimnAuto) const;
    ...
    */
    ...
    CzAIMNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzAIMNTracker> zaimnTracker;
    ...
    void setZWallet(CzAIMNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zaimnTracker = std::unique_ptr<CzAIMNTracker>(new CzAIMNTracker(strWalletFile));
    }

    CzAIMNWallet* getZWallet() { return zwalletMain; }

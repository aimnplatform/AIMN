...
VERSION=
commit=false
url=https://github.com/aimnproject/aimn
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the aimn, gitian-builder, gitian.sigs, and aimn-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/aimnproject/aimn
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit aimn=${COMMIT} --url aimn=${url} ../aimn/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../aimn/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/aimn-*.tar.gz build/out/src/aimn-*.tar.gz ../aimn-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit aimn=${COMMIT} --url aimn=${url} ../aimn/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/aimn-*.zip build/out/aimn-*.exe ../aimn-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit aimn=${COMMIT} --url aimn=${url} ../aimn/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/aimn-*.tar.gz build/out/aimn-*.dmg ../aimn-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit aimn=${COMMIT} --url aimn=${url} ../aimn/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../aimn/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/aimn-*.tar.gz build/out/src/aimn-*.tar.gz ../aimn-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../aimn/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/aimn-*win64-setup.exe ../aimn-binaries/${VERSION}
	    mv build/out/aimn-*win32-setup.exe ../aimn-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../aimn/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/aimn-osx-signed.dmg ../aimn-binaries/${VERSION}/aimn-${VERSION}-osx.dmg
	fi
	popd || exit

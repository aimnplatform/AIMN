...
====================================
This guide will show you how to build aimnd (headless client) for OSX.
...
5.  (Optional) You can also install aimnd to your path:
...
4. Enter "aimn-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `aimnd` for your own use.

aimnd/aimn-cli binaries are not included in the aimn-Qt.app bundle.

If you are building `aimnd` or `aimn-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Aimn-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./aimnd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./aimnd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=aimnrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Aimn/aimn.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Aimn/aimn.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Aimn/debug.log

Other commands:
-------

    ./aimnd -daemon # to start the aimn daemon.
    ./aimn-cli --help  # for a list of command-line options.
    ./aimn-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Aimn with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Aimn wallet:
...
You have the choice to build the GUI Aimn wallet as a Mac OSX app, described in “How to build the Aimn-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Aimn-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Aimn-Qt.dmg inside your Aimn folder. Open and install the wallet like any typical Mac app.
